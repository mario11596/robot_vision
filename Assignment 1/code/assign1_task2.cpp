...
StereoCalib(const vector<string>& imagelist, Size inputBoardSize, string type, float squareSize, float markerSize, cv::aruco::PredefinedDictionaryType arucoDict, string arucoDictFile, bool displayCorners = false, bool useCalibrated=true, bool showRectified=true)
{
    ...
    // Load XML file
    FileStorage fs1("input_params.xml", FileStorage::READ);
    if (!fs1.isOpened()) {
    ...
    fs1["camera_matrix_left"] >> cameraMatrix[0];
    fs1["distortion_coefficients_left"] >> distCoeffs[0];
    fs1["camera_matrix_right"] >> cameraMatrix[1];
    fs1["distortion_coefficients_right"] >> distCoeffs[1];
    
    fs1.release(); // Release the file
